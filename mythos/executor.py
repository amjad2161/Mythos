...
The executor is responsible for driving a single Task to completion by
running the agent's inner tool-calling loop until the task is marked done,
the tool ``finish`` is called, or the monitor raises an alert.
"""
...
FINISH_TOOL = "finish"
DECOMPOSE_TOOL = "decompose_tasks"   # pseudo-tool: agent decompose signal
...
class Executor:
    ...
    The executor does NOT own the memory, monitor, or registry – it operates
    on shared references provided by the agent.
    """
        ...
        while True:
            ...
            if response.has_tool_call:
                ...
                if tool_name == FINISH_TOOL:
                    ...
                    self._memory.add_message("assistant", f"[finish] {conclusion}")
                    return conclusion

                # Normal tool call
                result = self._registry.call(tool_name, tool_args)
                success = not result.startswith("ERROR:")
                self._monitor.record_tool_call(tool_name, success, result[:120])
